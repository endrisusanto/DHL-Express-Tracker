  ...
  const deleteShipmentFromDB = async (id: string) => {
      try {
        await fetch(getApiUrl(), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'delete_shipment', id })
        });
                                ...
                                <div key={item.id} className={`bg-white p-4 rounded-xl shadow-sm border border-gray-200 relative transition-all ${item.isCollected ? 'bg-gray-50/80 border-l-4 border-l-green-500' : ''}`}>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 border-t border-gray-100 pt-3">
                                        <button 
                                            ...
                                            className={`flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg text-xs font-bold border transition-all duration-300 ${
                                                item.isCollected 
                            ...
                            <table className="w-full min-w-[1000px]">
                                <thead>
                                    <tr className="bg-gray-50 border-b border-gray-200">
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-64">PIC Team</th>
                                    ...
                                    {shipments.map((item) => (
                                        <tr key={item.id} className={`group transition-colors ${item.isCollected ? 'bg-green-50/30' : 'hover:bg-yellow-50/30'}`}>
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col gap-2">
                                                    ...
                                                    <div className="relative flex items-center">
                                                        <UserPlus size={14} className="absolute left-2 text-gray-400 pointer-events-none" />
                                                        <input 
                                                            type="text" 
                                                            value={tempPicInput[item.id] || ''}
                                                            onChange={(e) => handlePicInputChange(item.id, e.target.value)}
                                                            onKeyDown={(e) => handlePicKeyDown(e, item.id)}
                                                            placeholder="Add PIC..."
                                                            className="w-full pl-7 pr-2 py-1 text-xs bg-gray-800 text-white placeholder-gray-400 border border-gray-700 rounded-md focus:ring-1 focus:ring-dhl-red focus:border-dhl-red outline-none hover:bg-gray-700 transition-colors"
                                                        />
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4">
                                                <button 
                                                    onClick={() => toggleCollected(item.id)}
                                                    className={`group relative w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg text-xs font-bold border transition-all duration-300 overflow-hidden ${
                                                        item.isCollected 
                            ...
                            <div className="space-y-6 flex-grow">
                                ...
                                <div className="pt-6 border-t border-gray-100 mt-auto">
                                    <button 
                                        ...
                                        className={`w-full group relative flex items-center justify-center gap-3 px-6 py-4 rounded-xl font-bold border-2 transition-all duration-300 overflow-hidden ${
                                            activeShipment.isCollected 
